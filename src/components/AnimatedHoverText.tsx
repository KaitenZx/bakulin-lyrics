...
const AnimatedHoverText: React.FC<AnimatedHoverTextProps> = ({ lines, className }) => {
  ...
  useEffect(() => {
    ...
    function runAnimation(item: HTMLElement, fromIndex: number) {
      const visibleWrappers = item.querySelectorAll('[data-visible] .letter-wrapper')
      const hiddenWrappers = item.querySelectorAll('[data-hidden] .letter-wrapper')

      gsap.to(visibleWrappers, {
        yPercent: 100,
        ease: 'back.out(2)',
        duration: 0.6,
      ...
      gsap.to(hiddenWrappers, {
        yPercent: 100,
        ease: 'back.out(2)',
        duration: 0.6,
        stagger: { each: 0.023, from: fromIndex },
        onComplete: () => {
          gsap.set(visibleWrappers, { clearProps: 'all' })
          gsap.set(hiddenWrappers, { clearProps: 'all' })
        },
      })
    }
    ...
    function onMouseOver(this: Element, ev: Event) {
      ...
      if (!gsap.isTweening(item.querySelectorAll('[data-visible] .letter-wrapper')) && item.classList.contains('hovered')) {
        item.classList.remove('hovered')
      }

      if (!(target && (target.classList.contains('letter') || target.classList.contains('letter-wrapper')))) return

      item.classList.add('hovered')
      const wrapper = target.classList.contains('letter-wrapper') ? target : target.parentElement
      const indexHover = Array.from(wrapper?.parentElement!.children || []).indexOf(wrapper!)
    ...
    function onMouseMove(this: Element, ev: MouseEvent) {
      ...
      if (gsap.isTweening(visibleWrappers)) return

      const rect = item.getBoundingClientRect()
      const x = ev.clientX - rect.left
      let closestIdx = 0
      let minDist = Infinity
      visibleWrappers.forEach((el, idx) => {
    ...
    function renderLetters(segments: Segment[]) {
      ...
      segments.forEach((seg) => {
        if (seg.isDot) {
          const idx = globalIdx++
          nodes.push(
            <span key={`wrapper-dot-${idx}`} className="letter-wrapper">
              <span key={`dot-${idx}`} className={['letter', 'dot'].join(' ')} aria-hidden />
            </span>,
          )
          return
        }
        ...
        chars.forEach((ch) => {
          ...
          const idx = globalIdx++
          nodes.push(
            <span key={`wrapper-${idx}`} className="letter-wrapper">
              <span
                key={`l-${idx}`}
                className={[
                  'letter',
